...
external buffer : (. stream('b)) => (. stream('a)) => stream(array('a)) =
  "default";
...
[@bs.send]
external apply :
  (concat, Js.Nullable.t('b), array(stream('a))) => stream('a) =
  "";
...
external dropRepeats :
  (. Js.Nullable.t(('a, 'a) => bool)) => (. stream('a)) => stream('a) =
  "default";

let dropRepeats = (~isEqual: option(('a, 'a) => bool)=?, (), source) =>
  (dropRepeats(. Js.Nullable.fromOption(isEqual)))(. source);

[@bs.module "xstream/extra/dropUntil"]
external dropUntil : (. stream('b)) => (. stream('a)) => stream('a) =
  "default";
...
external fromDiagram :
  (string, Js.Nullable.t(diagramOptions('a, 'b))) => stream('a) =
  "default";
...
external sampleCombine :
  (. stream('b)) => (. stream('a)) => stream(('a, 'b)) =
  "default";

let sampleCombine = (combined, source) =>
  (sampleCombine(. combined))(. source);

[@bs.module "xstream/extra/sampleCombine"]
external sampleCombine2 :
  (. stream('b), stream('c)) => (. stream('a)) => stream(('a, 'b, 'c)) =
  "default";

let sampleCombine2 = (combined1, combined2, source) =>
  (sampleCombine2(. combined1, combined2))(. source);

[@bs.module "xstream/extra/split"]
external split : (. stream('b)) => (. stream('a)) => stream(stream('a)) =
  "default";
